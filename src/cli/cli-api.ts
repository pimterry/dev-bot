import AwsSdk = require("aws-sdk");
import { CliArguments, CliAction } from "./arg-parsing";
import { buildBundle } from "../bundle";
import { AwsCredentials, LambdaConfig, AwsDeployer, AwsRoleCreator } from "../aws";

const DEFAULT_ROLE_NAME = "autogenerated-dev-bot-role";

export async function deploy(rootDirectory: string, // Absolute path to project
                             entryPoint: string, // Relative path in project to entrypoint
                             functionName: string,
                             region: string,
                             awsCredentials: AwsCredentials,
                             roleName: string = null): Promise<void> {
    let bundle = await buildBundle({ rootDirectory, entryPoint });

    let role = await new AwsRoleCreator(AwsSdk)
                        .createRole(roleName || DEFAULT_ROLE_NAME, awsCredentials);

    await new AwsDeployer(AwsSdk).deployLambdaBundle(bundle, {
        functionName,
        region,
        handler: "dev-bot-handler.handler",
        role
    }, awsCredentials);
}
